...
docker/scout-skills/rosbridge.py — only what a Hz sampler needs). One
websocket per call: a fresh socket means a rosbridge restart can never
strand this exporter with a dead subscription it thinks is live.
"""
...
class RosBridge:
    ...
    async def subscribe_collect(
        ...
        """Every message on `topic` for `duration` seconds — the raw material
        for a Hz measurement (len(msgs)/duration)."""
        sid = f"sub:{next(_ids)}"
        await self._ws.send(
            json.dumps(
