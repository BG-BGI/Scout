...
- **A single W cannot be correct on both carpet and hard floor** — scrub is strongly surface-dependent (see "Pivot duty ceiling"), so any chosen value is a compromise that is wrong somewhere. The permanent fix is fusing gyro yaw, after which the live gyro-to-odom ratio *is* the scrub factor on whatever floor you are on. `ros-humble-robot-localization` is **not** installed yet
...
If the `wheel_separation` work resumes, the calibrator worth rebuilding did this: pivot in place at several speeds in both directions, and compare gyro yaw against the driver's own `/odom` **pose** yaw, unwrapped rather than integrating the reported rate, so none of our own integration sits in the comparison. Worth reproducing from that design: report per-run scrub plus the implied `wheel_separation` and matching `max_angular_velocity` derate; name whichever gyro axis actually rotated most; flag a sign disagreement between odom and gyro; refuse to move when `/odom` or `/imu/data` is silent. Its `wheel_separation` parameter **must match what the driver is running**, since the answer is relative to the constant that produced `/odom`.
...
- Deployed controller is the **`roboclaw_driver` ROS 2 node** (wimblerobotics/Sigyn, built from the `kahleeeb3` fork in the Dockerfile), run via docker-compose; params in `scout/config/roboclaw.yaml`. Address 0x80, 115200 baud. There is no longer any standalone raw-UART bench path — `motor_test.py` was deleted, so open-loop duty commands now need a new tool.
